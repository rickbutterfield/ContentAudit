...
| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `MaxConcurrentCrawls` | `int` | `4` | The maximum number of crawls to perform at the same time. Must be between 1 and 20. |
...
| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `CrawlDelayMs` | `int` | `0` | Delay in milliseconds between crawl requests. Set to `0` for no delay. This can be overridden by the `Crawl-delay` directive in `robots.txt` if `RespectRobotsTxt` is enabled. Must be between 0 and 60000 (60 seconds). |
