...
export class ContentAuditAllPagesLinksWorkspaceViewElement extends UmbLitElement implements UmbWorkspaceViewElement {
	...
	#updateTableItems() {
		...
		const pageItems = this._links.slice(start, start + PAGE_SIZE);

		this._tableItems = pageItems.map((link) => ({
			id: link.unique,
			data: [
				{ columnAlias: 'url', value: link.url },
				{ columnAlias: 'type', value: html`${link.isExternal ? 'External' : 'Internal'}` }
			]
		}));
	}
	...
	#renderPagination() {
		const totalPages = Math.ceil(this._links.length / PAGE_SIZE);
