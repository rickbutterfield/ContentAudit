...
export class ContentAuditAllPagesImagesWorkspaceViewElement extends UmbLitElement implements UmbWorkspaceViewElement {
	...
	#updateTableItems() {
		...
		const pageItems = this._images.slice(start, start + PAGE_SIZE);

		this._tableItems = pageItems.map((image) => ({
			id: image.unique,
			data: [
				{ columnAlias: 'url', value: image.url },
				{ columnAlias: 'contentType', value: image.contentType }
			]
		}));
	}
	...
	#renderPagination() {
		const totalPages = Math.ceil(this._images.length / PAGE_SIZE);
