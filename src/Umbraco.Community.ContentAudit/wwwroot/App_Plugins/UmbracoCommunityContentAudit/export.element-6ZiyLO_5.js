import { UmbElementMixin as p } from "@umbraco-cms/backoffice/element-api";
import { LitElement as l, html as m, css as v, customElement as h } from "@umbraco-cms/backoffice/external/lit";
import { tryExecute as x } from "@umbraco-cms/backoffice/resources";
import { A as f } from "./index-Bkks3_By.js";
var E = Object.getOwnPropertyDescriptor, d = (t) => {
  throw TypeError(t);
}, _ = (t, e, o, a) => {
  for (var r = a > 1 ? void 0 : a ? E(e, o) : e, i = t.length - 1, c; i >= 0; i--)
    (c = t[i]) && (r = c(r) || r);
  return r;
}, b = (t, e, o) => e.has(t) || d("Cannot " + o), C = (t, e, o) => e.has(t) ? d("Cannot add the same private member more than once") : e instanceof WeakSet ? e.add(t) : e.set(t, o), y = (t, e, o) => (b(t, e, "access private method"), o), s, u;
let n = class extends p(l) {
  constructor() {
    super(), C(this, s);
  }
  render() {
    return m`
		    <umb-workspace-editor headline="Export" .enforceNoFooter=${!0}>
                <div id="main">
                    <uui-box headline="CSV export">
                        <p>Export your latest audit run to a CSV file.</p>
                        <uui-button look="primary" label="Export CSV" @click=${() => y(this, s, u).call(this)}></uui-button>
                    </uui-box>
                </div>
		    </umb-workspace-editor>
	    `;
  }
};
s = /* @__PURE__ */ new WeakSet();
u = async function() {
  const { data: t, error: e } = await x(this, f.export());
  if (t) {
    const o = new Blob([t], { type: "text/csv;charset=utf-8;" }), a = URL.createObjectURL(o), r = document.createElement("a");
    r.href = a, r.download = "content-audit-export.csv", document.body.appendChild(r), r.click(), document.body.removeChild(r), URL.revokeObjectURL(a);
  }
  e && console.error(e);
};
n.styles = [
  v`
            :host {
                display: block;
            }

            #main {
                padding: var(--uui-size-space-5);
            }
        `
];
n = _([
  h("content-audit-export")
], n);
const L = n;
export {
  n as ContentAuditExportElement,
  L as default
};
//# sourceMappingURL=export.element-6ZiyLO_5.js.map
