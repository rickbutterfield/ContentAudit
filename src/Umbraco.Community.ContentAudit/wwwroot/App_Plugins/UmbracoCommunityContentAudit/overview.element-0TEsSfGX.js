...
import { LitElement as T, repeat as U, html as u, nothing as C, css as L, state as p, customElement as I } from "@umbraco-cms/backoffice/external/lit";
import { C as M } from "./index-BW3lOCKc.js";
import { UmbModalToken as N, UMB_MODAL_MANAGER_CONTEXT as W } from "@umbraco-cms/backoffice/modal";
import "./run-warning-modal.element-6Knxqvtw.js";
...
A = function() {
  var e, t, a, r, i, l;
  if (this.scanRunning) {
    const b = this.crawlData.length, D = this.crawlData.filter((s) => s.crawled && !s.external && !s.asset).length, x = this.crawlData.filter((s) => s.crawled && s.external && !s.asset).length, E = this.crawlData.filter((s) => s.crawled && s.asset).length, k = this.crawlData.filter((s) => s.blocked).length;
    return u`
  ...
  if (this._latestAuditOverview !== void 0)
    return u`
                <uui-box headline="Latest audit" class="span-2" style="${((e = this._latestAuditOverview) == null ? void 0 : e.runDate) != null || this.scanRunning ? "--uui-box-default-padding: 0;" : ""}">
                    <div slot="header">
                        ${((t = this._latestAuditOverview) == null ? void 0 : t.runDate) != null ? this.localize.date((a = this._latestAuditOverview) == null ? void 0 : a.runDate, { dateStyle: "long", timeStyle: "short" }) : C}
                    </div>
...
//# sourceMappingURL=overview.element-0TEsSfGX.js.map
