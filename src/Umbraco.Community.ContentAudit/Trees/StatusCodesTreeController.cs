...
{
    [Tree(Constants.SectionAlias, "status-codes", TreeGroup = Constants.Trees.Audit.Group, SortOrder = 1)]
    [PluginController("UmbracoCommunityContentAudit")]
    public class StatusCodesTreeController : TreeController
    {
        private readonly IMenuItemCollectionFactory _menuItemCollectionFactory;

        public StatusCodesTreeController(ILocalizedTextService localizedTextService,
        ...
        protected override ActionResult<TreeNode?> CreateRootNode(FormCollection queryStrings)
        {
            ...
            if (root is not null)
            {
                root.RoutePath = $"{Constants.SectionAlias}/status-codes/overview";
                root.Name = "Status Codes";
                root.Icon = "icon-stop-alt";
