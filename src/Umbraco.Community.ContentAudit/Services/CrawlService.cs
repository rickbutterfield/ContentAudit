                ...
                var analysisData = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        pageAnalysis.SeoData = new SeoDto
                        {
                            ...
                            H1 = h1Text,
                            H2s = (await Task.WhenAll(h2Elements.Select(h => h.TextContentAsync()))).Where(text => !string.IsNullOrWhiteSpace(text)).Select(text => text!.Trim()).ToList(),
                            H3s = (await Task.WhenAll(h3Elements.Select(h => h.TextContentAsync()))).Where(text => !string.IsNullOrWhiteSpace(text)).Select(text => text!.Trim()).ToList(),
